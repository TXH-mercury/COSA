...
def preprocess_gts(train_desc,tokenizer):
    annfile = json.load(open(train_desc))
    # annfile = annfile['annotations']
    output = defaultdict(list)
    for anno in annfile:
        caption = anno['caption'] if 'caption' in anno else anno['desc']
        # caption = tokenizer.encode(caption)[1:-1]
...
class Scorer(object):
    def __init__(self, train_desc, tokenizer):
        super(Scorer, self).__init__()
        self.scorers = []
        #self.weights = cfg.SCORER.WEIGHTS
        #self.gts = pickle.load(open(cfg.SCORER.GT_PATH, 'rb'), encoding='bytes')

        document_frequency, ref_len = precompute_df_reflen_for_cider(train_desc, tokenizer)

        self.gts = preprocess_gts(train_desc, tokenizer)
...
def precompute_df_reflen_for_cider(train_desc, tokenizer):
    gts = []
    ...
    train_descs = json.load(open(train_desc))
    id2anns = defaultdict(list)
    for a in train_descs:
        # if a['video_id'] in ids:
        caption = a['caption'] if 'caption' in a else a['desc']
        # id2anns[a['video_id']].append(tokenizer.encode(caption)[1:-1])
        id2anns[a['video_id']].append(tokenizer.encode(caption))  
