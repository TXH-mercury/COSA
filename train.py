...
from train_utils import initialize, load_from_pretrained_dir,  conduct_train, \
                                set_parallel_optimizer_and_apex, zero_shot_evaluation, get_args, create_train_dataloaders, load_from_resume, \
                                    create_val_dataloaders, set_dropout, compute_video_sample_num
from test import get_model_attr
from model.pretrain import COSA
import os 
import json
import torch
import torch.nn.functional as F

def main(opts, input_args):

    ### init 
    initialize(opts)
    
 
    compute_video_sample_num(opts)

    checkpoint = {}

    if opts.pretrain_dir is not None:

        checkpoint = load_from_pretrained_dir(opts, input_args)
    ...
    ### initialize model 
    model = COSA.from_pretrained(opts,checkpoint)


    ### set parallel, optimizer  and apex 
    model, optimizer = set_parallel_optimizer_and_apex(model, opts, checkpoint_optim)
    ### create datasets and dataloader
    tokenizer = get_model_attr(model,'multimodal_encoder').tokenizer ### for scst
    train_loader, scorer = create_train_dataloaders(opts, tokenizer)
    val_loaders = create_val_dataloaders(opts)
